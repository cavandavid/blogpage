...
* [`src/cavan_blog/`](src/cavan_blog/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cavan_blog/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `cavan-blog` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cavan_blog/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cavan-blog.example
  (:require [cavan-blog.config :as config])
